...
func showUsage() {
	...
	fmt.Println("  install --symlink-prefix <path> <pkg>")
	fmt.Println("                    Install with symlink prefix stripping for chroot")
	...
	fmt.Println("")
	fmt.Println("  # Install with symlink prefix stripping (for chroot)")
	fmt.Println("  chisel install --symlink-prefix /tmp vim")
	fmt.Println("")
...
func handleInstall(args []string) {
	if len(args) < 1 {
		...
		fmt.Fprintln(os.Stderr, "  --symlink-prefix  Strip prefix from symlink targets (e.g., /tmp for chroot)")
		os.Exit(1)
	}
