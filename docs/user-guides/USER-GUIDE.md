...
The `--chroot` flag enables running packages inside a chroot environment by doing TWO important things:
...
1. **Symlink Location**: `--chroot=/tmp/demo` → symlinks created in `/tmp/demo/usr/bin/`
